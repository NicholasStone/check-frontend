...
function Location(props) {
  ...
  function afterNameDrag() {
    ...
    dispatch(updateAutosLocation({
      ...location,
      name: {...location.name, x: location.name.x + tmp[0], y: location.name.y + tmp[1]}
    }))
  }
  ...
  function afterInvariantDrag() {
    ...
    dispatch(updateAutosLocation({
      ...location,
      invariant: {...location.invariant, x: location.invariant.x + tmp[0], y: location.invariant.y + tmp[1]}
    }))
  }
  ...
  return (
    <>
      ...
      <Draggable onStart={() => selectedTool === 'select'} onStop={afterLocationDrag} position={{x: 0, y: 0}}>
        <use ref={locationRef} id={location.id} className='circle' x={location.x - 12} y={location.y - 12} width={24}
             height={24} xlinkHref={init ? "#icon-circle-start" : "#icon-circle"}
             onDoubleClick={() => {
      ...
      <Draggable onStart={() => selectedTool === 'select'} onStop={afterInvariantDrag} position={{x: 0, y: 0}}>
        <text ref={invariantRef} x={location.invariant.x} y={location.invariant.y}
              fill="#DA70D6">{location.invariant.content}</text>
      </Draggable>
    </>
  )
}
